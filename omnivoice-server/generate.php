...
function debugLog($step, $status, $detail = '') {
    global $debugSteps, $debugStart, $requestId;
    $debugSteps[] = [
        'time' => date('H:i:s'),
        'step' => $step,
        'status' => $status,
...
debugLog('Input recebido', 'info', "texto: " . mb_substr($texto, 0, 50) . " | idioma: $idioma | steps: $numStep | reqId: $requestId");
...
function downloadRefAudio($url, $name) {
    ...
    // ===== DOWNLOAD COM VALIDACAO DE INTEGRIDADE =====
    // Se o audio de referencia chegar corrompido, o TTS gera engasgo/lixo.
    // Fazemos ate 3 tentativas para garantir integridade.
    $dlResult = downloadWithValidation($url, $name, 3);

    if (isset($dlResult['error'])) {
        debugLog('Download ref audio', 'error', 'falha apos validacao: ' . $dlResult['error']);
        return null;
    }

    $tempFile = $dlResult['path'];
    $refSha256 = $dlResult['sha256'];
    debugLog('Download ref audio', 'ok', round($dlResult['size'] / 1024) . "KB | SHA256: " . substr($refSha256, 0, 12) . '...');
    ...
    if ($trimmedFile && $trimmedFile !== $tempFile) {
        ...
        // Re-validar apos trim
        $postTrim = validateAudioFile($tempFile, pathinfo($name, PATHINFO_EXTENSION), 'ref apos trim');
        if (!$postTrim['valid']) {
            debugLog('Trim audio', 'error', 'audio corrompido apos trim: ' . $postTrim['error']);
            if (file_exists($tempFile)) unlink($tempFile);
            return null;
        }
        debugLog('Trim audio', 'ok', round($postTrim['size'] / 1024) . 'KB (max ' . MAX_REF_AUDIO_SECONDS . 's) | SHA256: ' . substr($postTrim['sha256'], 0, 12) . '...');
    } elseif ($trimmedFile === false) {
        debugLog('Trim audio', 'warn', 'Falha no trim, usando original (validado OK)');
    }
...
function uploadToHF($filePath, $fileName, $hfUrl) {
    ...
    $result = uploadWithValidation($filePath, $fileName, $hfUrl);
    if ($result) {
        debugLog('Upload para HF', 'ok', $result);
        return $result;
    }
    debugLog('Upload para HF', 'error', 'falha na resposta do Gradio');
    return null;
}
...
function runGeneration($gradioData, $refAudioFile, $refAudioName, $hfUrl) {
    if ($refAudioFile && file_exists($refAudioFile)) {
        // Validar integridade do ref ANTES de enviar ao TTS
        $refExt = strtolower(pathinfo($refAudioName, PATHINFO_EXTENSION));
        $refCheck = validateAudioFile($refAudioFile, $refExt, 'ref antes do upload');
        if (!$refCheck['valid']) {
            debugLog('Upload', 'error', 'ref audio corrompido: ' . $refCheck['error']);
            return ['audioUrl' => null, 'error' => 'Audio de referencia corrompido: ' . $refCheck['error']];
        }
        debugLog('Upload', 'info', 'ref validado OK | SHA256: ' . substr($refCheck['sha256'], 0, 12) . '...');
...
// ===================== BAIXAR AUDIO GERADO COM VALIDACAO =====================
debugLog('Download audio gerado', 'info', 'baixando com validacao de integridade...');
$genExt = strtolower(pathinfo($audioUrl, PATHINFO_EXTENSION));
if (empty($genExt)) $genExt = 'wav';

$dlResult = downloadWithValidation($audioUrl, 'generated.' . $genExt, 3);

if (isset($dlResult['error'])) {
    monitorEndGen($monitorFile, $genId, false);
    returnError('Audio gerado corrompido ou vazio: ' . $dlResult['error'] . ' (tunnel instability)');
}

$tempAudioFile = $dlResult['path'];
$audioSize = $dlResult['size'];
$genSha256 = $dlResult['sha256'];
debugLog('Download audio gerado', 'ok', round($audioSize / 1024) . "KB | SHA256: " . substr($genSha256, 0, 12) . '...');
...
// ===================== RETORNAR =====================
debugLog('FINAL', 'ok', 'audio pronto via PHP DIRECT (SSE + trim + validacao)');

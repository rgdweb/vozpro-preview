...
export async function GET() {
    ...
    if (phpRes.ok) {
      ...
      if (phpData.problemas && Array.isArray(phpData.problemas)) {
        ...
        for (const p of phpData.problemas) {
          ...
          const isPythonTtsWarn = /python_tts.*n[oã]o|TTS.*parado|Native.*Parado/i.test(p)
  ...
  result.checks.vercel = {
    ...
    hf_space_url: process.env.HF_SPACE_URL ? 'configurada (nao usada - sistema native)' : 'vazia (usa tunnel)',

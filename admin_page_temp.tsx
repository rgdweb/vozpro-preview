...
import {
  AudioWaveform, LogOut, Plus, Trash2, Edit, Upload, Music, Mic, Zap,
...
export default function AdminDashboard() {
    ...
    try {
      ...
      try {
        ...
        if (speakersRes.ok) {
          ...
          setSpeakerVarIds(new Set((speakersData || []).map((s: { speakerFile: string }) => s.speakerFile)))
        }
      } catch {}
      if (managedCatRes.ok) {
  ...
  const handleToggleSpeaker = async (variation: VoiceVariation, voiceName: string) => {
    const speakerFileKey = slugify(voiceName) + '_' + slugify(variation.label) + '.pt'
                                  ...
                                  <div key={v.id} className={`flex items-center justify-between rounded-lg border px-3 py-2 ${!v.active ? 'border-slate-800 bg-slate-900/20 opacity-60' : (v.refAudioPath || v.refAudioServerUrl) ? 'border-emerald-800/40 bg-emerald-900/10' : 'border-amber-800/40 bg-amber-900/10'}`}>
                                    ...
                                    <div className="flex items-center gap-1 shrink-0">
                                      ...
                                      <Button
                                        ...
                                        className={`h-7 w-7 ${speakerVarIds.has(slugify(voice.name) + '_' + slugify(v.label) + '.pt') ? 'text-emerald-400 hover:text-emerald-300 hover:bg-emerald-900/30' : 'text-amber-400/70 hover:text-amber-300 hover:bg-amber-900/30'}`}
                                        title={speakerVarIds.has(slugify(voice.name) + '_' + slugify(v.label) + '.pt') ? 'Desativar Clonagem Rapida' : 'Ativar Clonagem Rapida (Locutor Oficial)'}
                                      >

#!/usr/bin/env python3
"""Add defaultSpeed column using psql directly."""
import paramiko
...
out, _ = run(client, f"grep DATABASE_URL {REMOTE}/.env 2>/dev/null")
print("DB URL found:", "YES" if "postgresql" in out else "NO")

# Add column via prisma db execute with --schema flag
...
# Verify
out, err = run(client, f'cd {REMOTE} && echo \'SELECT column_name, data_type FROM information_schema.columns WHERE table_name = \'\\'"'"'VoiceVariation\'\\'"'"\' AND column_name = \'\\'"'"'defaultSpeed\'\\'"'"\';\' | npx prisma db execute --schema prisma/schema.prisma --stdin 2>&1', timeout=20)

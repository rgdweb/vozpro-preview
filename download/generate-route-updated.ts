...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const { variationId, text, language, trackId, trackVolume, speed, numStep } = body
    ...
    const refText = (variation as Record<string, unknown>).refText as string || ''
    ...
    debug.log('Payload', 'info',
      `cfg:1.5 | speed:${payload.speed} | steps:${payload.num_step} | ref_text:${(payload.ref_text as string).substring(0, 40)}...`)

...
debugLog('Input', 'info', "modo: $mode | texto: " . mb_substr($texto, 0, 50) . " | lang: $idioma | steps: $numStep | reqId: $requestId");
...
function uploadToGradio($filePath, $fileName, $baseUrl) {
    ...
    $result = uploadWithValidation($filePath, $fileName, $baseUrl);
    if ($result) {
        debugLog('Upload Gradio', 'ok', $result);
        return $result;
    }
    debugLog('Upload Gradio', 'error', 'falha na resposta do Gradio');
    return null;
}
...
for ($attempt = 0; $attempt < $maxRetries && !$audioUrl; $attempt++) {
    ...
    if ($mode === 'clone' && $tempRefFile && file_exists($tempRefFile)) {
        // Validar integridade do ref ANTES de enviar
        $refExt = strtolower(pathinfo($refAudioName, PATHINFO_EXTENSION));
        $refCheck = validateAudioFile($tempRefFile, $refExt, 'ref antes do upload');
        if (!$refCheck['valid']) {
            debugLog('Upload', 'error', 'ref audio corrompido: ' . $refCheck['error']);
            $lastError = 'Audio de referencia corrompido: ' . $refCheck['error'];
            continue;
        }
...
debugLog('Download audio', 'info', 'baixando audio gerado com validacao de integridade...');
...
debugLog('Download audio', 'ok', round($audioSize / 1024) . "KB | SHA256: " . substr($genSha256, 0, 12) . '...');
...
debugLog('FINAL', 'ok', 'OmniVoice via PHP DIRECT (validado)');

...
//
// CORRECOES APLICADAS (15/05/2026):
// - Adicionada stripSSML() para remover tags SSML antes de enviar ao TTS
// - CURLOPT_ENCODING => '' em TODOS os curls (impede gzip/brotli em audio binario)
// - SSE timeout aumentado de 300s para 600s (textos longos nao estouram)
// - memory_limit aumentado para 512M (base64 de audio longo precisa mais memoria)
// - Timeout de download de audio aumentado para 180s
// - Timeout de submit job aumentado para 90s
...
// ===================== STRIP SSML (defesa) =====================
// CORRECAO: Esta funcao faltava no servidor. Sem ela, tags SSML eram enviadas
// como texto literal para o TTS, causando garbling e leitura de tags.
function stripSSML($text) {
    if (!is_string($text)) return '';
    if (!preg_match('/<[a-z][^>]*>/i', $text)) return $text;
    // Remove todas as tags XML/SSML
    $r = preg_replace('/<[^>]+>/', '', $text);
    // Remove entidades XML
    $r = html_entity_decode($r, ENT_QUOTES | ENT_XML1, 'UTF-8');
    return trim(preg_replace('/\s+/', ' ', $r));
}

// ===================== LIMPAR TEXTO (defesa extra) =====================
// Remove caracteres problemáticos que podem causar garbling
function cleanText($text) {
    if (!is_string($text)) return '';
    // Remove caracteres de controle invisíveis (exceto quebra de linha e tab)
    $text = preg_replace('/[\x00-\x08\x0B\x0C\x0E-\x1F\x7F]/', '', $text);
    // Normaliza quebras de linha
    $text = str_replace(["\r\n", "\r"], "\n", $text);
    // Remove linhas vazias excessivas (mas mantém quebras simples)
    $text = preg_replace('/\n{3,}/', "\n\n", $text);
    return trim($text);
}
...
function downloadRefAudio($url, $name) {
    ...
    curl_setopt_array($ch, [
        ...
        CURLOPT_TIMEOUT => 60,
        // CORRECAO: CURLOPT_ENCODING => '' impede que o curl descomprima gzip automaticamente
        // e corrompa o audio binario
        CURLOPT_ENCODING => '',
        CURLOPT_SSL_VERIFYPEER => false,
    ]);
    $dlOk = curl_exec($ch);
...
function uploadToGradio($filePath, $fileName, $baseUrl) {
    ...
    curl_setopt_array($ch, [
        ...
        CURLOPT_RETURNTRANSFER => true,
        CURLOPT_TIMEOUT => 60,
        // CORRECAO: CURLOPT_ENCODING => '' em upload tambem
        CURLOPT_ENCODING => '',
        CURLOPT_SSL_VERIFYPEER => false,
    ]);
    $resp = curl_exec($ch);
...
function submitJob($baseUrl, $endpoint, $gradioData) {
    ...
    curl_setopt_array($ch, [
        ...
        CURLOPT_RETURNTRANSFER => true,
        // CORRECAO: Timeout aumentado de 60s para 90s
        CURLOPT_TIMEOUT => 90,
        CURLOPT_CONNECTTIMEOUT => 20,
        // CORRECAO: CURLOPT_ENCODING => '' no submit
        CURLOPT_ENCODING => '',
        CURLOPT_SSL_VERIFYPEER => false,
    ]);
    $resp = curl_exec($ch);
...
// CORRECAO: Timeout padrao aumentado de 300s para 600s
function streamResult($baseUrl, $endpoint, $eventId, $timeoutSec = 600) {
    ...
    curl_setopt_array($ch, [
        ...
        // CORRECAO CRITICA: CURLOPT_ENCODING => '' no SSE stream
        // Sem isso, dados binarios de audio podem ser corrompidos pelo gzip
        CURLOPT_ENCODING => '',
        CURLOPT_HTTPHEADER => [
...
if ($mode === 'clone') {
    ...
    $gradioData = [
        ...
        (int)$numStep,                     // ns (numStep)
        2.0,                               // gs (CFG)
        true,                              // dn (denoise)
        (float)$speed,                     // sp (speed)
        null,                              // du (duration, null = auto)
        true,                              // pp (preprocess)
        false,                             // po (postprocess) - CORRECAO: false evita cortes no final
    ];
...
for ($attempt = 0; $attempt < $maxRetries && !$audioUrl; $attempt++) {
    ...
    // CORRECAO: Timeout de stream aumentado para 600s
...
curl_setopt_array($ch, [
    CURLOPT_FILE => $fp,
    CURLOPT_FOLLOWLOCATION => true,
    // CORRECAO: Timeout aumentado de 120s para 180s
    CURLOPT_TIMEOUT => 180,
    // CORRECAO: CURLOPT_ENCODING => '' para nao corromper audio com descompressao
    CURLOPT_ENCODING => '',
    CURLOPT_SSL_VERIFYPEER => false,
]);
$dlOk = curl_exec($ch);

...
// DEBUG TEMP
file_put_contents('/tmp/tunnel-debug.log', date('Y-m-d H:i:s') . ' | ' . $rawInput . "\n", FILE_APPEND);
...
$nativePayload = [
    ...
    'ref_audio_url'        => (!empty($input['ref_audio_url']) ? $input['ref_audio_url'] : ($input['referenceAudioUrl'] ?? '')),
    ...
    'ref_text'             => (!empty($input['ref_text']) ? $input['ref_text'] : (!empty($input['refText']) ? $input['refText'] : 'texto de referencia para clonagem de voz')),
...
if ((($result['status'] ?? '') === 'ok' || ($result['status'] ?? '') === 'success') && !empty($result['audio_base64'])) {

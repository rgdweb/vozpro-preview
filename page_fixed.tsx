...
/**
 * Normaliza volume RMS de um AudioBuffer para um target dB.
 * Evita saltos de volume entre chunks que causam sensação de "corte".
 */
function rmsNormalize(audioBuffer: AudioBuffer, targetDb = -16): AudioBuffer {
  ...
  if (rms < 0.001) return audioBuffer // silêncio, não normalizar

  const gain = targetLinear / rms
  // Limitar ganho (evitar amplificar ruído demais)
  const safeGain = Math.min(gain, 4.0)
  // Aplicar ganho suave (não clipar)
  const ctx = new AudioContext({ sampleRate: audioBuffer.sampleRate })
  const normalized = ctx.createBuffer(1, data.length, audioBuffer.sampleRate)
  const out = normalized.getChannelData(0)
  for (let i = 0; i < data.length; i++) {
    out[i] = Math.max(-1, Math.min(1, data[i] * safeGain))
  }
  ctx.close()
  return normalized
}
...
/**
 * Trim de silêncio client-side — APENAS do final do áudio.
 * Corta o silêncio excessivo que o VozPro adiciona no final (52s → ~14s).
 * Conservador: threshold mais alto (0.03) + buffer de 200ms para não cortar sílabas.
 */
function trimEndSilence(audioBuffer: AudioBuffer, threshold = 0.03, keepBufferMs = 200): AudioBuffer {
  const data = audioBuffer.getChannelData(0)
  const sampleRate = audioBuffer.sampleRate
  const bufferSamples = Math.floor(sampleRate * (keepBufferMs / 1000))

  // Encontrar fim do áudio (varrer de trás pra frente)
  let end = data.length - 1
  for (let i = data.length - 1; i >= 0; i--) {
    if (Math.abs(data[i]) >= threshold) {
      end = Math.min(data.length - 1, i + bufferSamples)
      break
    }
  }

  if (end >= data.length - 1) return audioBuffer // sem silêncio no final

  const trimmedLength = end + 1
  const ctx = new AudioContext({ sampleRate })
  const trimmed = ctx.createBuffer(1, trimmedLength, sampleRate)
  const trimmedData = trimmed.getChannelData(0)
  for (let i = 0; i < trimmedLength; i++) {
...
export default function VozProClient() {
  console.log('[VozPro] VERSION: 2026-06-05-v3-clone_fast-fix | BUILD:', 'pfBL7J19WNkm8TbsEN54')
  ...
  // Fetch Locutores Oficiais
  useEffect(() => {
    fetch('/api/speakers')
      .then(r => r.ok ? r.json() : [])
      .then(data => {
        if (Array.isArray(data)) setSpeakers(data.filter((s: { isActive: boolean }) => s.isActive))
        console.log('[VozPro] Speakers loaded:', data.filter((s) => s.isActive).length, 'speakers')
      })
      .catch(() => {})
  }, [])
  ...
  const [selectedSpeakerFile, setSelectedSpeakerFile] = useState<string | null>(null)
  const [speakers, setSpeakers] = useState<Array<{ id: string; name: string; avatarUrl: string | null; speakerFile: string; isActive: boolean }>>([])
  ...
  }, [selectedVoiceId, selectedVoice, selectedSpeakerId])
  ...
  const handleGenerate = useCallback(async () => {
    ...
    if (voiceMode === 'clone' && !selectedVariationId && !uploadedVoiceUrl && !selectedSpeakerId) {
    ...
    try {
      ...
      // ===== LOCUTOR OFICIAL (clone_fast) — via /api/generate (Next.js backend) =====
      // Bypass PHP proxy: o Next.js monta o payload correto e chama a GPU direto
      console.log('[DEBUG-GENERATE] selectedSpeakerFile:', selectedSpeakerFile, '| selectedSpeakerId:', selectedSpeakerId, '| type:', typeof selectedSpeakerFile); if (selectedSpeakerFile) {
        console.log('[F5-TTS] Locutor Oficial (clone_fast):', selectedSpeakerFile)
        res = await fetch('/api/generate', {
          ...
          body: JSON.stringify({
            ...
            speed,
            numStep,
            speakerFile: selectedSpeakerFile,
            voiceMode: 'clone_fast',
          }),
        ...
        <div className="container mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="text-xs text-slate-500 hover:text-rose-400 transition-colors duration-200 px-2 py-1 rounded-lg hover:bg-rose-500/10"
              title="Sair"
            >
              Sair
            </button>
          ...
          <div className="lg:col-span-3 space-y-5">
                ...
                {voices.length === 0 ? (
                  ...
                  <>
                    <button
                      ...
                      className="flex items-center gap-2 text-sm text-violet-400 hover:text-violet-300 transition-colors duration-200 hover:bg-violet-500/5 -ml-2 px-2 py-1 rounded-lg"
                    >
                    ...
                    {voices.filter(v => v.category === selectedVoiceCategory).length === 0 ? (
                      ...
                      <div className="space-y-1">
                        {voices.filter(v => v.category === selectedVoiceCategory).map((voice) => {
                          ...
                          return (
                            <div key={voice.id} className="rounded-xl overflow-hidden transition-all duration-300">
                              {/* Voice card — clique expande/colapsa e seleciona */}
                              <button
                                onClick={() => {
                                  setSelectedVoiceId(voice.id)
                                  setSelectedSpeakerId(null)
                                  setSelectedSpeakerFile(null)
                                ...
                                <div className="px-2 pb-2 pt-1 ml-4 border-l-2 border-violet-500/30 space-y-1 animate-in slide-in-from-top-1 duration-150">
                                  {voice.variations.map((v) => {
                                    ...
                                    return (
                                      <button
                                        key={v.id}
                                        onClick={(e) => { e.stopPropagation(); setSelectedVariationId(v.id); setSelectedSpeakerId(null); setSelectedSpeakerFile(null) }}
                  ...
                  <>
                    ...
                    {voices.filter(v => !v.category).length > 0 && (
                      <div className="mb-4">
                        <p className="text-xs text-slate-500 mb-2 font-medium tracking-wide uppercase">Sem categoria</p>
                      ...
                      <div>
                        ...
                        <div className="flex flex-wrap gap-2">
                          {selectedVoice.variations.map((v) => {
                            const varAudioUrl = toProxyAudioUrl(v.refAudioServerUrl || v.refAudioPath || '')
                            return (
                              <button key={v.id} onClick={() => { setSelectedVariationId(v.id); setSelectedSpeakerId(null); setSelectedSpeakerFile(null) }} className={`px-4 py-2 rounded-full border text-sm transition-all duration-300 flex items-center gap-1.5 ${selectedVariationId === v.id ? 'border-violet-500 bg-violet-500/20 text-violet-200' : 'border-white/10 bg-white/5 text-slate-400 hover:border-white/20'}`}>
                                <VoicePreviewButton
            ...
            <Card className="bg-white/5 border-white/10 backdrop-blur">
              ...
              <CardContent className="space-y-4">
                ...
                {/* ===== LOCUTORES OFICIAIS (clone_fast via embedding) ===== */}
                {voiceMode === 'clone' && speakers.length > 0 && (console.log('[DEBUG-RENDER] Rendering Locutores section | voiceMode:', voiceMode, '| speakers:', speakers.length, '| selectedSpeakerId:', selectedSpeakerId, '| selectedSpeakerFile:', selectedSpeakerFile), true) && (
                  <div className="space-y-2 pt-2 border-t border-white/10">
                        ...
                        <button
                          ...
                          onClick={() => {
                            if (selectedSpeakerId === speaker.id) {
                              ...
                              setSelectedSpeakerFile(speaker.speakerFile); console.log('[DEBUG-SPEAKER-CLICK] speakerFile set to:', speaker.speakerFile, '| speaker obj:', JSON.stringify({id:speaker.id,name:speaker.name,speakerFile:speaker.speakerFile}))
                              setSelectedVoiceId('')
                              setSelectedVariationId('')
                          ...
                          className={`p-3 rounded-xl border text-center transition-all duration-200 ${
                            selectedSpeakerId === speaker.id
                              ? 'border-emerald-500 bg-emerald-500/20 ring-1 ring-emerald-500/30'
                              : 'border-white/10 bg-white/5 hover:border-white/20 hover:bg-white/10'
                          }`}
                        >
                          <div className="text-lg mb-1">{speaker.avatarUrl ? (
                            <img src={speaker.avatarUrl} alt={speaker.name} className="w-8 h-8 rounded-full mx-auto object-cover" />
                          ) : (
                            '🎙️'
                          )}</div>
                          <div className={`text-xs font-medium truncate ${
                            selectedSpeakerId === speaker.id ? 'text-emerald-200' : 'text-slate-400'
                          }`}>
                            {speaker.name}
                          </div>
                          <div className="text-[10px] text-slate-600">Embedding pronto</div>
                        </button>
                      ))}
                    </div>
                    {selectedSpeakerId && (
                      <p className="text-xs text-emerald-400/70 text-center">
                        Locutor selecionado — síntese acelerada via embedding
                      </p>
                    )}
                  </div>
                )}
                  ...
                  {tracks.length === 0 ? (
                    ...
                    <>
                      <button
                        ...
                        className="flex items-center gap-2 text-sm text-purple-400 hover:text-purple-300 transition-colors duration-200 hover:bg-purple-500/5 -ml-2 px-2 py-1 rounded-lg"
                      >
                        <ChevronLeft className="w-4 h-4" />
            ...
            <details className="group">
              <summary className="flex items-center gap-2 cursor-pointer text-sm text-slate-400 hover:text-violet-300 transition-colors duration-200 py-1">
          ...
          <div className="lg:col-span-2 space-y-5">
            ...
            <details open={debugOpen} className="group">
              <summary
                ...
                className="flex items-center gap-2 cursor-pointer text-sm text-slate-500 hover:text-slate-300 transition-colors duration-200 py-1"
              >
              ...
              <Card className="mt-2 bg-white/5 border-white/10 backdrop-blur">
                <CardContent className="pt-5 space-y-3">
                  ...
                  {(lastGenResponse as Record<string, unknown>)?.debug && 
                    ...
                    <div className="space-y-2">
                      <div className="flex items-center justify-between">
                        <span className="text-xs font-semibold text-slate-200">Log de Geração</span>
                        <div className="flex items-center gap-2">
                          ...
                          <Button
                            variant="ghost"
                            size="sm"
                            className="h-5 px-1.5 text-slate-500 hover:text-white hover:bg-white/5 transition-colors duration-200"
                      ...
                      <div className="space-y-1 max-h-60 overflow-auto">
                        {(((lastGenResponse as Record<string, unknown>)?.debug as Record<string, unknown>)?.steps as Array<Record<string, string>>).map((step: Record<string, string>, i: number) => (
                          <div key={i} className={`flex items-start gap-2 text-[11px] font-mono px-2 py-1 rounded-lg ${
      ...
      <div className="fixed bottom-0 left-0 right-0 z-50">
        ...
        <div className="relative z-50 bg-slate-900/95 border-t border-white/10 px-4 py-3">
          ...
          <Button
            ...
            disabled={isGenerating || !text.trim() || (voiceMode === 'clone' && !selectedVariationId && !uploadedVoiceUrl && !selectedSpeakerId)}
            ...
            title={
              !selectedVariationId && !uploadedVoiceUrl && !selectedSpeakerId && voiceMode === 'clone'

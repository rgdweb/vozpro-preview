<?php
// generate-direct.php - Geracao de voz TTS via F5-TTS (chamada DIRETA do browser)
...
//   Resolvido bug onde HF_SPACE_URL = '' (vazio) era tratado como URL valida
// - CORRIGIDO: CORS headers com header_remove + set (evita duplicata com .htaccess)
// - cleanText() remove caracteres de controle invisiveis
// - SSE timeout 600s para textos longos
// - Extensao detectada do audio gerado (nao forca .wav)
// - Trim audio ref max 10s (evita CUDA OOM na RTX 3060 12GB)
// - CURLOPT_ENCODING => '' em TODOS os curl (bloqueia gzip que corrompe audio via tunnel)
...
$instruct = $input['instruct'] ?? null;  // null por padrao (igual ao original)
...
// Usa getTtsUrl() que tenta: TUNNEL_URL > HF_SPACE_URL > get_tunnel.php > fallback
$hfUrl = getTtsUrl();
...
/**
 * Trim audio para max N segundos (usa trim_audio.py)
 */
...
/**
 * Upload de arquivo local para HF Space / OmniVoice
 */
...
// Montar dados do Gradio (parametros restaurados do ORIGINAL funcionando)
$gradioData = [
    $texto,
    'Auto',  // Auto detecta idioma (interface do VozPro usa Auto)
    [
        'path' => $refAudioPath ?? '',
    ...
    $refText ?? '',  // refText: texto de referencia do admin (vazio = Whisper auto)
    $instruct,       // instruct: null (interface do VozPro envia null!)
    (int)$numStep,
    (float)$guidanceScale,
    true,      // denoise
    (float)$speed, // speed como float
    null,      // duration: null = auto
    true,      // preprocess
    true       // postprocess
];

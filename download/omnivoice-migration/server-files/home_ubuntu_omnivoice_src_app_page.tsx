              ...
              <CardContent className="space-y-4">
                ...
                {voiceMode === 'design' && (
                  <div className="space-y-2 pt-2 border-t border-white/10">
                    ...
                    />
                    {/* Voice Design Attribute Buttons */}
                    <div className="space-y-2 mt-2">
                      <p className="text-[10px] text-slate-500 font-medium">Toque para adicionar atributos:</p>
                      {[
                        { group: 'Genero', items: ['female', 'male'] },
                        { group: 'Idade', items: ['child', 'teenager', 'young adult', 'middle-aged', 'elderly'] },
                        { group: 'Tom', items: ['very low pitch', 'low pitch', 'moderate pitch', 'high pitch', 'very high pitch'] },
                        { group: 'Estilo', items: ['whisper'] },
                        { group: 'Sotaque', items: ['american accent', 'australian accent', 'british accent', 'canadian accent', 'chinese accent', 'indian accent', 'japanese accent', 'korean accent', 'portuguese accent', 'russian accent'] },
                      ].map((group) => (
                        <div key={group.group}>
                          <span className="text-[10px] text-slate-600">{group.group}:</span>
                          <div className="flex flex-wrap gap-1 mt-0.5">
                            {group.items.map((item) => {
                              const lower = voiceDesignInstruct.toLowerCase()
                              const alreadyHas = lower.includes(', ' + item.toLowerCase() + ',') || lower.startsWith(item.toLowerCase() + ',') || lower.endsWith(', ' + item.toLowerCase()) || lower === item.toLowerCase()
                              return (
                                <button
                                  key={item}
                                  type="button"
                                  onClick={() => {
                                    ...
                                    const current = voiceDesignInstruct.trim()
                                    setVoiceDesignInstruct(current ? current + ', ' + item : item)
                                  }}
                                  className={`px-2 py-0.5 rounded text-[11px] transition-all ${
                                    alreadyHas
                                      ? 'bg-violet-600/40 text-violet-300 border border-violet-500/40 cursor-default'
                                      : 'bg-white/5 text-slate-400 border border-white/10 hover:bg-violet-500/20 hover:text-violet-300 hover:border-violet-500/30 cursor-pointer'
                                  }`}
                                  disabled={alreadyHas}
                                >
                                  {alreadyHas && '✓ '}{item}
